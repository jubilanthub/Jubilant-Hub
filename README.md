...
<title>Michael Maluleka Vision Hub</title>
<style>
...
#searchInput{width:90%;padding:12px;margin:10px auto;display:block;border:none;border-radius:20px;outline:none;}
...
.product{background:#151515;border-radius:10px;overflow:hidden;}
...
.footer-box{background:#111;padding:20px;margin:15px;border-radius:10px;}
...
<div id="loader">
<img src="png.png" alt="Logo" onerror="this.src='https://via.placeholder.com/140x140/D4AF37/000?text=MMVH'">
...
<div class="topbar">
<img src="png.png" alt="Logo" onerror="this.src='https://via.placeholder.com/70x70/D4AF37/000?text=MMVH'">
...
<h2>About Us</h2>
<p>Michael Maluleka Vision Hub is committed to providing quality products, reliable service, and affordable delivery options.</p>
...
<script>
// Hide loader after 2.5s no matter what
setTimeout(() => {
...
// Use placeholder images if yours don't exist yet
const placeholder = 'https://via.placeholder.com/300x300/222/D4AF37?text=Product';

for(let i=1;i<=80;i++){
categories.dresses.push({n:'Dress '+i,p:'R150',img:placeholder});
}
for(let i=1;i<=6;i++){
categories.shoes.push({n:'Shoe '+i,p:'R250',img:placeholder});
}
for(let i=1;i<=30;i++){
categories.men.push({n:'Men Outfit '+i,p:'R350',img:placeholder});
}
for(let i=1;i<=30;i++){
categories.sneakers.push({n:'Sneaker '+i,p:'R300',img:placeholder});
}
for(let i=1;i<=6;i++){
categories.slippers.push({n:'Slipper '+i,p:'R160',img:placeholder});
}
for(let i=1;i<=30;i++){
categories.other.push({n:'Other '+i,p:'R200',img:placeholder});
}
...
gallery.innerHTML += `
<div class="product">
<img src="${item.img}" alt="${item.n}" onerror="this.src='${placeholder}'">
<div class="info">
<h3>${item.n}</h3>
